...
# Sempre rodar do root do repo
ROOT="$(git rev-parse --show-toplevel)"
cd "$ROOT"

echo "PWD=$(pwd)"
echo "== CREATE: VPS pack (public) =="

# Pastas
mkdir -p ssot/vps ssot/policies ssot/terminal ssot/device
mkdir -p templates/vps/fail2ban templates/vps/auditd templates/vps/cron
mkdir -p docs/ssot_public docs/runbooks
mkdir -p scripts/vps
mkdir -p evidence/public

# ===== SSOT Targets =====
cat > ssot/vps/VPS_SECURITY_TARGETS.yaml <<'EOF'
